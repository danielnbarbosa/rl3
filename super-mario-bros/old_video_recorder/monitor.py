...
import old_video_recorder.stats_recorder as stats_recorder
import old_video_recorder.video_recorder as video_recorder
import old_video_recorder.atomic_write as atomic_write
import old_video_recorder.closer as closer
from old_video_recorder.json_utils import json_encode_np
...
def collapse_env_infos(env_infos, training_dir):
    ...
    for other in env_infos[1:]:
        if first != other:
            raise error.Error("Found two unequal env_infos: {} and {}. This usually indicates that your training directory {} has commingled results from multiple runs.".format(
                first, other, training_dir))

    for key in ["env_id", "gym_version"]:
        if key not in first:
            raise error.Error("env_info {} from training directory {} is missing expected key {}. This is unexpected and likely indicates a bug in gym.".format(
