...
class Agent:
    ...
    def train(self, filename=None):
        ...
        # create save paths
        training_run_path = Path('training_runs/' + device.type + '-' + ENV.split('/')[1] + '-' + str(datetime.now()).replace(' ', '-'))  # unique folder per training run
        ...
        while train_steps <= TRAIN_STEPS_MAX:
            ...
            # track current lives to support when a life is lost, save it in replay memory as terminal state
            lives = 5
            done = False
            while not done:
                # choose action
